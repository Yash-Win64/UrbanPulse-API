...
def collect_all_data():
    ...
    try:
        weather_url = (
            f"https://api.openweathermap.org/data/2.5/weather?q={CITY}&units=metric&appid={OPENWEATHER_KEY}"
        )
    ...
    try:
        ...
        if not hourly:
            ...
            db.add(entry)
            db.commit()
            log.info(f"Air Quality stored | {CITY}: PM2.5={pm25} µg/m³, AQI={latest_aqi}")
    except Exception as e:
